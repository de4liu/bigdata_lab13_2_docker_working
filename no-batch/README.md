...
```bash
docker build -t msba6331-mlops-docker:no-batch .
```

You can use the `-t` flag to specify the name of the image and its tag. As you saw earlier the tag comes after the colon so in this case the name is `msba6331-mlops-docker` and the tag is `no-batch`.
...
```bash
docker run --rm -p 80:80 msba6331-mlops-docker:no-batch
```
